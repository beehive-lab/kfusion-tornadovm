...
# the argfile was generated by tornado --generate-argfile, which (per its
# wrapper script) always runs under "$JAVA_HOME/bin/java" - so the JVM that
# consumes the argfile must be that same JDK, or its JDK-version-specific
# flags/module-path can mismatch whatever JDK a stale PATH entry hands back
# for bare "java".
