...
__kernel void optMapReduce(__global void *_heap_base, ulong _stack_base)
{
  ...
  __global ulong *slots = ((__global ulong *) (((ulong) _heap_base) + _stack_base));
    ulong4 args = vload4(0,&slots[4]);
    
    const int sizeX = (int) args.s2;
    const int sizeY = (int) args.s3;
    
    // device remaps memory space if printf is used
    // here we find the difference between our value of a0 and the remapped value
    // this can then be added onto each parameter.
    //a0 = (ulong) _heap_base + 0x5f60c0;
    a1 = 0;//a0 - args.x;
    
    // a0  =  trackingResults + 32;
    // a1  =  *((__global ulong *) a0);
    
    args += a1;
    
    ulong a3 = args.y + 32;
    ulong a4 =  *((__global ulong *) a3);
    __global float* trackingResults = (__global float *)  ( a4 + a1+ 24);
...
//    for(i=0;i<32;i+=4){
//            vstore4(vload4(0,&privateOutput[i]),0,&localOutput[index + i]);
//    }
