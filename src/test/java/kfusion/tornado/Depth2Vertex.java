...
//		d2v.loadFromFile("opencl/depth2vertex.cl");
		...
		//@formatter:off
		depthToVertex = new TaskGraph()
			.streamIn(depth[0], invK)
			.add(GraphicsMath::depth2vertex,vertex[0],depth[0],invK)
			.streamOut(vertex[0])
		...
		//@formatter:off
		vertexToNormal = new TaskGraph()
		    .streamIn(vertex[0])
			.add(GraphicsMath::vertex2normal,normal[0], vertex[0])
			.streamOut(normal[0])
			.mapAllTo(config.getTornadoDevice());
		//@formatter:on
	...
	public void testDepth2vertex() {
		...
		for (int index = 0; index < frames.length; index += 3) {
			...
			if (maxULP > config.getMaxULP()) errors++;
