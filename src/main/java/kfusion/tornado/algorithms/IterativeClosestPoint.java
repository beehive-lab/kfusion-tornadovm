...
public class IterativeClosestPoint {
	...
	private static void reduceSumWithError(@Reduce final float[] sums, float error, int startIndex, int N,
			float[] value) {
	...
	public static void reduceArrayValues(final float[] sums, final int startIndex, final ImageFloat8 trackingResults,
			int resultIndex) {
	...
	// public static void reduceInner(final float[] sums, final ImageFloat8
	// trackingResults,
	// int resultIndex) {
	//
	// final int jtj = 7;
	...
	public static void trackPose(final ImageFloat8 results, final ImageFloat3 verticies, final ImageFloat3 normals,
			final ImageFloat3 referenceVerticies, final ImageFloat3 referenceNormals, final Matrix4x4Float currentPose,
			final Matrix4x4Float view, final float distanceThreshold, final float normalThreshold) {
		...
		for (@Parallel int y = 0; y < results.Y(); y++) {
			for (@Parallel int x = 0; x < results.X(); x++) {
				...
				if (normals.get(x, y).getX() == Constants.INVALID) {
					...
					final Float2 projectedPixel = Float2
							.add(Float2.mult(projectedPos.asFloat2(), 1f / projectedPos.getZ()), 0.5f);

					boolean isNotInImage = (projectedPixel.getX() < 0)
							|| (projectedPixel.getX() > (referenceVerticies.X() - 1)) || (projectedPixel.getY() < 0)
					...
					if (isNotInImage) {
						...
						if (referenceNormal.getX() == Constants.INVALID) {
							...
							final Float3 diff = Float3.sub(referenceVerticies.get(refPixel.getX(), refPixel.getY()),
									projectedVertex);
							...
							if (Float3.length(diff) > distanceThreshold) {
								...
								if (Float3.dot(projectedNormal, referenceNormal) < normalThreshold) {
									...
									final Float8 tracking = new Float8(referenceNormal.getX(), referenceNormal.getY(),
											referenceNormal.getZ(), b.getX(), b.getY(), b.getZ(),
	...
	public static <T extends KfusionConfig> boolean estimateNewPose(final T config, final TrackingResult result,
			final ImageFloat8 trackingResults, final Matrix4x4Float currentPose, final float icpThreshold) {
	...
	public static <T extends KfusionConfig> boolean estimateNewPose(final T config, final TrackingResult result,
			final float[] icpResults, final Matrix4x4Float currentPose, final float icpThreshold) {
