...
public class IterativeClosestPoint {
    ...
    public static void mapReduce(final FloatArray output, final FloatArray trackingResults, final int numElements) {
        ...
        for (@Parallel int i = 0; i < numThreads; i++) {
            ...
            for (int j = i; j < numElements; j += numThreads) {
                final int offset = j * TRACK_STRIDE;
                ...
                output.set(startIndex + 0 + 1, output.get(startIndex + 0 + 1) + (error * trackingResults.get(offset + 0)));
                output.set(startIndex + 1 + 1, output.get(startIndex + 1 + 1) + (error * trackingResults.get(offset + 1)));
                output.set(startIndex + 2 + 1, output.get(startIndex + 2 + 1) + (error * trackingResults.get(offset + 2)));
                output.set(startIndex + 3 + 1, output.get(startIndex + 3 + 1) + (error * trackingResults.get(offset + 3)));
                output.set(startIndex + 4 + 1, output.get(startIndex + 4 + 1) + (error * trackingResults.get(offset + 4)));
                output.set(startIndex + 5 + 1, output.get(startIndex + 5 + 1) + (error * trackingResults.get(offset + 5)));

                accumulateJtJRows01(output, jtj, trackingResults, offset);
                accumulateJtJRows2345(output, jtj, trackingResults, offset);
    ...
    // Split out of mapReduce() purely for readability, and to let that task
    // compile under TornadoVM's default inlining policy - see the mapReduce()
    // comment above.
    //
    ...
    // trackingResults, offset) dispatcher calling both halves (665 nodes,
    // worse than the original monolithic method's 603) and calling both
    // halves from a separate reduceValues() helper (638 nodes) - only the
    // actual root method is exempt from this cap, so both halves must be
    // called directly from mapReduce() itself with no intermediate method in
    // between.
    //
    ...
    private static void accumulateJtJRows01(final FloatArray sums, final int jtj, final FloatArray trackingResults, final int offset) {
    ...
    private static void accumulateJtJRows2345(final FloatArray sums, final int jtj, final FloatArray trackingResults, final int offset) {
